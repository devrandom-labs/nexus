...
- **`schema_version` is `NonZeroU32` — an adapter's column MUST hold the full range.** First cut used postgres `INTEGER` (`i32`) and rejected values above `i32::MAX` as corrupt. The shared `assert_event_stream_conformance` suite (which feeds a boundary `schema_version` near `u32::MAX`) caught this immediately — a legal `SchemaVersion` was being refused. Fixed to `BIGINT` (`u32` widens to `i64` infallibly). **This is a concrete win for #213:** the second adapter + the canonical conformance suite together caught a silent data-rejection bug that fjall (which stores the raw `u32`) never exposed. Worth a trait-doc note: an adapter must persist the entire `SchemaVersion` domain, and a fixed-width column narrower than `u32` is wrong.
