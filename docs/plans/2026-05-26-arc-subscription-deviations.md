...
## [PR 4 | Task 1] — Decision REVISED: Option C (no code change; sub-trait + all impls stay as load-bearing combinator API)
- **Type**: open-point-resolved / decision-reversed-after-controller-error
- **Plan said**: PR4 acceptance criteria (plan lines 1354–1359) offered three options. Option A: delete `OwnedEventStream`, simplify `IntoStream`. Option B: keep but simplify; verify no dead-code paths remain. Option C: no change, document why and skip PR.
- **Initial controller decision (later reversed)**: Option B — delete `MapErr<S, F, E2>: OwnedEventStream<M>` impl on YAGNI/grep-empty grounds; document `_marker`. The evaluator subagent verified zero in-tree callers of `.map_err(...).into_stream()`; CLAUDE.md rule 4 ("justify every type") and YAGNI were cited.
- **Why that decision was wrong**: The controller and evaluator both misunderstood the design intent of the `stream/` module. Per the user (Joel) on 2026-05-27, the module is:
  1. **The on-ramp to `futures::Stream` combinators.** Owning combinators (`Map`, `TryMap`, `MapErr`) materialize the lending GAT `Item<'a>` into an owned `T`, then `.into_stream()` bridges into `futures::Stream` where the entire futures combinator ecosystem (`.take_while`, `.filter`, `.chain`, `.then`, `.buffer_unordered`, `try_collect`, etc.) becomes available. **Each owning-combinator impl of `OwnedEventStream` is one lane on that on-ramp.**
  2. **Destined for extraction into its own crate outside nexus.** The user is building stream as a standalone generic lending-iterator-with-yield combinator library; nexus is its primary (and current sole) consumer. The trait surface IS the design.
  3. **Also the backbone of nexus.** It serves nexus's event-sourcing, subscription, and projection layers — but those are users of the surface, not the surface itself.

  Under that framing, `MapErr: OwnedEventStream` is **load-bearing API**, not speculative dead code. Without it, users cannot chain `.try_map(decode).map_err(into_app_error).into_stream()` — error conversion would have to fuse into the decode step, coupling error policy to decoding. Empty grep proves no in-tree caller has needed that chain yet; it does NOT prove the chain is dead. The trait surface IS the contract.

- **Actually did**: Reverted to **Option C**. No code change in PR4. The `OwnedEventStream` sub-trait and ALL of its impls (`Map`, `TryMap`, `MapErr`) stay verbatim. `IntoStream` and its `_marker` field stay verbatim. The pre-existing `--all-targets` clippy debt in `futures_bridge_tests.rs` is out of scope (logged in PR3 verification deviation; not touched here).
- **Lesson for the controller**: For combinator-library code, the YAGNI burden is inverted — symmetric trait surfaces are the design intent; asymmetric gaps are the deviation requiring justification, not the symmetric impls. The "extraction-bound + nexus-backbone" dual status means even impls without current in-tree consumers can be load-bearing API. The plan's PR4 framing ("evaluate whether sub-trait is needed") presupposed the question; the answer is: the sub-trait IS the canonical owning-combinator API of the bridge, and the question is malformed. See memory `project-stream-is-generic-combinator-library` and `feedback-no-yagni-combinator-impls`.
- **Acceptance criteria for PR4 (now)**:
  - No source code changed.
  - Either: (a) skip PR entirely — delete the `refactor/arc-subscription-pr4` branch, OR (b) ship a docs-only PR that updates the `OwnedEventStream` doc-comment to explicitly state it is the on-ramp to `futures::Stream` and that the `Map`/`TryMap`/`MapErr` impls are the canonical owning-combinator surface — so future readers (and evaluators) don't propose deletion.
  - Scope between (a) and (b) is a user decision logged in a follow-up entry.
- **Open risks**: None — Option C is the no-op outcome the plan anticipated.
- **Date / PR link**: 2026-05-27, branch `refactor/arc-subscription-pr4` (decision reversal before any code commit)
