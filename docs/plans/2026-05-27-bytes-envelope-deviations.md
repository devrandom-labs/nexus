...
## [PR 3 | scope] — One pre-existing test fix bundled with the docs PR

- **Type**: added
- **Plan said**: "No code changes in this PR. If you find a code bug while writing docs, log it for a separate PR."
- **Actually did**: Fixed `attack_store_error_conflict_preserves_info` in `crates/nexus-store/tests/adversarial_property_tests.rs` as part of the PR3 commit. Strategy: replaced bare integer ranges with `prop_oneof![Just(None), Just(Version::new(1)), Just(Version::new(u64::MAX)), (2u64..u64::MAX).prop_map(Version::new)]` for both `expected` and `actual`. Body now asserts `msg.contains(&format!("{expected:?}"))` and `msg.contains(&format!("{actual:?}"))` rather than the raw integer `.to_string()`. Stress-tested at 8 × 2000 cases — all pass.
- **Reason**: The test had a latent bug surfaced randomly by proptest seeds. The original strategy was `expected in 0..10000u64`, and `Version::new(0)` returns `None` (Version is `NonZeroU64`). The error format uses `{expected:?}` which renders `None` as the string `"None"` — but the test asserted `msg.contains(&expected.to_string())`, i.e. `"0"`. About 40% of fresh proptest seeds eventually sample 0, triggering the failure. The pre-commit hook (`nix flake check`) runs the test in release profile and would have randomly failed every commit attempt on this branch. Splitting into a separate PR would have meant: open mini-PR, wait for review, merge, rebase PR3, retry — and during that window all main commits would also be flaky. Fixing in-PR is one commit, immediately unblocks every contributor, and the fix is self-contained (one test, no production-code touch). User explicitly authorized this scope expansion in the PR3 planning conversation.
- **Impact**: One test-only file change rides along with the docs commit. Adds boundary coverage (`None`, `Version::new(1)`, `Version::new(u64::MAX)`) that the original strategy lacked entirely, satisfying `feedback_exhaustive_testing`'s "ranges must include boundaries via prop_oneof!" rule. Asserts the actual semantic invariant — "the Debug rep of each side appears verbatim in the rendered error" — instead of the looser "some textual form of the integer appears", which makes the test resilient to future error-format tweaks that change number rendering but preserve the `{:?}` shape.
- **Date / PR link**: 2026-06-04, PR TBD
