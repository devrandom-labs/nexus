...
// =============================================================================
// apply multiple events in a loop
// =============================================================================

#[test]
fn apply_multiple_events_in_loop() {
...
fn current_version_after_rehydrate_plus_new_events() {
    ...
    agg.apply(TEvent::Added(Added("d".into())));
    agg.apply(TEvent::Removed(Removed));
...
fn take_then_apply_then_take_again() {
    ...
    // First batch
    agg.apply(TEvent::Added(Added("first".into())));
    ...
    agg.apply(TEvent::Added(Added("second".into())));
    agg.apply(TEvent::Added(Added("third".into())));
...
// =============================================================================
// version increments exactly by 1 per apply, even with many uncommitted
// =============================================================================
...
fn version_increments_by_one_each_apply_with_many_uncommitted() {
    ...
    for i in 0..5 {
        ...
        agg.apply(TEvent::Added(Added(format!("item-{i}"))));
        let post = agg.current_version();
        assert_eq!(
            post.as_u64(),
            pre.as_u64() + 1,
            "After apply #{i}, current_version should be exactly pre + 1"
        );
    }
